...
This avoids the "dumb zone" that happens when one agent tries to hold an entire
feature in its head.
...
wrong direction. If the plan is wrong, both agents build the wrong thing in
parallel — twice the waste.

### 3. Split into scoped briefs
Create one brief per agent. Each brief must:
...
**Plan quality test:** if the brief says "build the frontend for this feature" —
that's a wish. If it says "create `components/widget.js`, render a list from the
`/api/widgets` endpoint (shape: `{id, name, status}`), follow the card pattern in
`components/item.js:24-45`" — that's executable.
...
Each agent starts a fresh session with only its scoped brief loaded. Don't reuse
the research session. Fresh context = smart zone.
...
- **Don't force parallelism.** If the work is tightly coupled, keep it in one
  agent. Forced splits create merge conflicts and coordination overhead.
- **Commit before switching.** Changes leak between workspaces if uncommitted.
- **The shared plan is the contract.** If one agent deviates, update the plan.
- **Sub-agents are not roles.** Think "agent with context about files A, B, C"
  not "frontend agent." The split is about context, not identity.
